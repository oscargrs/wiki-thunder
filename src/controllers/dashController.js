...
function getStats(req, res) {
    ...
    dashboardModel.getStats()
        .then(
            function (resultadoStats) {
                console.log(`\nResultados encontrados: ${JSON.stringify(resultadoStats)}`);

                dashboardModel.getTopVehicles()
                    .then(
                        function (resultadoTopVehicles) {
                            console.log(`\nTop veículos: ${JSON.stringify(resultadoTopVehicles)}`);

                            dashboardModel.getFavoritesByNation()
                                .then(
                                    function (resultadoByNation) {
                                        console.log(`\nFavoritos por nação: ${JSON.stringify(resultadoByNation)}`);

                                        dashboardModel.getFavoritesByType()
                                            .then(
                                                function (resultadoByType) {
                                                    console.log(`\nFavoritos por tipo: ${JSON.stringify(resultadoByType)}`);

                                                    dashboardModel.getNationPreference()
                                                        .then(
                                                            function (resultadoNationPref) {
                                                            ...
                                                            function (erro) {
                                                                console.log("\nErro em getNationPreference:", erro.sqlMessage);
